...
class GA:
   ...
   def createGui(self, winningTour, distance):
		Map = Tk()
		canv = Canvas(Map, width=1000, height=800, bg="white")
		Map.title("Traveling Salesman -Genetic")
		Map.geometry("1600x1000")

		for city in winningTour:
			city.x *= 5
			city.y *= 5
...
if __name__ == '__main__':
   ...
   cityList = ga.readInCities()
   
   for i in range(0, len(cityList)):
			tourmanager.addCity(cityList[i])
   ...
   ga.createGui(pop.getFittest(), pop.getFittest().getDistance())
